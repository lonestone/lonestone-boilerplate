...
The preferred path is: merge, deploy, leave the flag off. Production already follows release tags, so an unfinished set can sit on `main` without shipping. The flag covers the case where the unfinished work is *inside* a release you otherwise want.
