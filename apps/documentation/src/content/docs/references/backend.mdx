...
- Use Better Auth for authentication via `@thallesp/nestjs-better-auth`
- Routes are protected by a **global** `AuthGuard` — mark public routes with `@AllowAnonymous()`, optional auth with `@OptionalAuth()`
- Use `@Session()` to access the current session; use `@Roles()` / `@OrgRoles()` for plugin-based RBAC
- Validate user permissions in services when business rules go beyond role metadata
...
export class UserController {
  @Get('me')
  getMe(@Session() session: LoggedInBetterAuthSession) {
    return session.user
  }
}
