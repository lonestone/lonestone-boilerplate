...
not inspect a bucket until `STORAGE_ENABLED=true`. The low-level storage module exposes a service
to domain modules, while domain modules such as Posts own HTTP routes, database metadata, and
authorization.
