...
v1.0.0 already shipped `auth.cli.mts`, but `knip.json` still listed `auth.cli.ts`. v1.1.0 corrects that entry so Knip does not warn on a missing file or miss the real CLI script.
...
Run `pnpm boilerplate upgrade record --id v1.1.0/point-knip-at-auth-cli-mts --applied` after validation passes, or record it as skipped with a reason.
