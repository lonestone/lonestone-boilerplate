...
v1.1.0 found that cookie 1.x must stay on 1.1.1 so Express (`parse`/`serialize`) and Astro ≤7.1.1 (`parseCookie`) remain compatible when `shamefully-hoist=true`. Astro is pinned to 7.1.1 in the documentation app for the same reason. This is a two-line compatibility pin, not an unbounded dependency upgrade.
...
- Catalog family version alignment — `v1.1.0/align-shared-dependency-versions`.
...
Run `pnpm boilerplate upgrade record --id v1.1.0/pin-cookie-override-for-hoist --applied` after validation passes, or record it as skipped with a reason.
