...
- Other `frontend` catalog entries (React, Tailwind, TanStack Query) — `v1.1.0/align-shared-dependency-versions`.
- i18n language-key identity — `v1.1.0/unify-i18n-language-keys`.
...
Run `pnpm boilerplate upgrade record --id v1.1.0/migrate-react-router-8 --applied` after validation passes, or record it as skipped with a reason.
