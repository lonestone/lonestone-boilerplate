...
v1.1.0 moves the boilerplate compiler baseline from TypeScript 5.9 to 6.0.3. TypeScript 6 deprecates `baseUrl`; without `ignoreDeprecations: "6.0"` and a pnpm allowance for `typescript@6`, `pnpm typecheck` and installs fail even when the rest of the catalog is unchanged.
...
- Other catalog families (`frontend`, `auth`, remaining `build` entries) — those are `v1.1.0/align-shared-dependency-versions`.
...
Run `pnpm boilerplate upgrade record --id v1.1.0/adopt-typescript-6 --applied` after validation passes, or record it as skipped with a reason.
