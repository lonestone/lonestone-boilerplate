...
4. **Diff first — never retype.** For each file the intention references, run `diff <file> .boilerstone/upgrade/reference/target/<file>` before editing. No project-specific delta → copy the staged reference verbatim. Project deltas → keep them and apply only the reference-side hunks. Elsewhere, make the smallest safe change and preserve project-specific behavior; avoid cosmetic edits.
